...
with mlflow.last_active_run() as parent_run:
    for i in range(0, len(models)):
        with mlflow.start_run(
            ...
            #mlflow.h2o.log_model(h2o_model=models[i],artifact_path=str(experiment_id)+"/"+str(models[i].getTrainingParams()['model_id']))
            #mlflow.h2o.save_model(h2o_model=models[i],path="s3://mlflow/"+str(experiment_id)+"/"+str(models[i].getTrainingParams()['model_id']))
